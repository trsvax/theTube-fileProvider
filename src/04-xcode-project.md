...
This gives them:
- Shared Keychain access (JWT + secret readable by all three)
- Shared container (if you ever need local state between app and extensions)
- Shared UserDefaults (preferences visible to extensions)
...
Same as above — needs Keychain access to read the JWT and fire a capture.
...
No Swift Package Manager dependencies. No CocoaPods. No Carthage. Three system frameworks, zero third-party code.
...
On iOS: share a photo from Camera Roll → Share → Tube → fires a capture with the image URL. On macOS: share a URL from Safari → Share → Tube → fires a link capture.

Same `TubeRequest.fire("share/add", ...)` as `send-tube capture`. One code path, three surfaces (CLI, share sheet, FileProvider writes if we ever add them).
...
prev: file-provider
next:
The Xcode project is the part that can't be a `# src` block — it's generated by GUI. But the decisions (app groups, entitlements, bundle IDs, what targets exist) are worth documenting. This post is the recipe for recreating the project from scratch. The Share Extension is the second capture surface after `send-tube` — easier than Shortcuts because it's just "show up in the share sheet for URLs and images, fire a tubeRequest." Same protocol, different input surface.
